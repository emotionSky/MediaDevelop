...
PlayThread::PlayThread(QObject *parent) : QObject(parent)
{
    ...
    connect(this, &PlayThread::ExistThreadSignal, this, &PlayThread::Stop);
}
...
bool PlayThread::GetStatus() const
{
    return m_bRunning;
}
...
void PlayThread::Start()
{
    ...
    m_bRunning = true;
    m_pThread = new std::thread(&PlayThread::Run, this);
}
...
void PlayThread::Run()
{
    ...
    while (m_bRunning)
    {
        m_len = file.read((char*)data, buf_len);
